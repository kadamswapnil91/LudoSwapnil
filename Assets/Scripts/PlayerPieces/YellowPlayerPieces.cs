...
public class YellowPlayerPieces : PlayerPiece, IPointerDownHandler
{
   ...
   public void OnMouseDown1(int playerId, int figureId, int pos)
    {
        ...
        if (SameMarker.Instance.getCountStep(pos) == 0)
        {
            ...
            //  rollingDice.YellowPlayerStopAnimaton();
            //    MakePlayerReadyToMove(playerId, figureId, pathParent.yellowPathPoints);
       ...
       if(SameMarker.Instance.getCountStep(pos) > 0 && isPathPointsAvailableToMove(GameManager.gm.numberOfStepsToMove,SameMarker.Instance.getCountStep(pos),pathParent.yellowPathPoints) && GameManager.gm.isReadyToMove && !GameManager.gm.isSixCountGraterThanTwo)
       {
            sendMqttMessage(playerId, figureId, pos);
            rollingDice.YellowPlayerStopAnimaton();
          setMarker(playerId,figureId,pos);
    ...
    public void OnPointerDown(PointerEventData eventData)
    {
      ...
      int pos = 0; 
        ...
        if(eventData.pointerCurrentRaycast.gameObject.name == "YellowPlayerPieces1")
        {
              playerId = 1;
              figureId = 1;
              pos = 0;
        }else if(eventData.pointerCurrentRaycast.gameObject.name == "YellowPlayerPieces2")
        {
              playerId = 1;
              figureId = 2;
              pos = 1;
        }else if(eventData.pointerCurrentRaycast.gameObject.name == "YellowPlayerPieces3")
        {
              playerId = 1;
              figureId = 3;
              pos = 2;
        }else if(eventData.pointerCurrentRaycast.gameObject.name == "YellowPlayerPieces4")
        {
              playerId = 1;
              figureId = 4;
              pos = 3;
        ...
        {
            if (GameManager.gm.playerTurn == 1 && !winnerResult.isShown() && !exitManager.isShown() && SameMarker.Instance.getPlayerArray()[0].player_name == Credentials.email && !GameManager.gm.isAutomaticallyMovePlayerPiece)
            {
        ...
        {
            if (GameManager.gm.playerTurn == 1 && !winnerResult.isShown() && !exitManager.isShown() && !GameManager.gm.isAutomaticallyMovePlayerPiece)
                OnMouseDown1(playerId, figureId, pos);
        }
    }
 ...
 public int getNewPosition(int pos)
    ...
    public void setMarker(int playerId, int figureId, int pos)
    {
      ...
      canMove = true;
      MoveSteps(playerId, figureId,pathParent.yellowPathPoints, isKillingList);
